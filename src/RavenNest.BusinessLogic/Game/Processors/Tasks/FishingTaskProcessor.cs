...
#warning TODO: Allow for getting resources if you have bonus increased your level?
                if (!TryGetIsland(state.Island, out var island) || islandLevelRequirements[island][RavenNest.Models.Skill.Fishing] > skills.FishingLevel)
                    return;
                ...
                Drops.TryDropItem(this, logger, gameData, inventoryProvider, session, character, skills.FishingLevel, state.TaskArgument);
            });
        }
    }
}
